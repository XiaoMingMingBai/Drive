...
void led_set_value(int which, int status)
{
    switch (which) {
    case LED1:
        status == LED_ON ? (gpioe->ODR |= (1 << 10)) : (gpioe->ODR &= ~(1 << 10));
        break;
    case LED2:
        status == LED_ON ? (gpiof->ODR |= (1 << 10)) : (gpiof->ODR &= ~(1 << 10));
        break;
    case LED3:
        status == LED_ON ? (gpioe->ODR |= (1 << 8)) : (gpioe->ODR &= ~(1 << 8));
        break;
    }
}
